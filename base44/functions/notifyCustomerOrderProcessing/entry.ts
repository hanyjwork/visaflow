...
Deno.serve(async (req) => {
    try {
        ...
        // Only send email if status changed to processing
        if (order.status !== 'processing' || oldOrder?.status === 'processing') {
            return Response.json({ message: 'Status not changed to processing' });
        }
        ...
        const emailResult = await resend.emails.send({
            ...
            subject: `🚀 Your Order is Being Processed - #${order.tracking_number}`,
            html: `
                <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                    <div style="background: linear-gradient(135deg, #2563eb 0%, #1d4ed8 100%); padding: 30px; text-align: center; border-radius: 8px 8px 0 0;">
                        <h1 style="color: white; margin: 0; font-size: 28px;">Processing Your Order</h1>
                    </div>
                    ...
                    <div style="background: #ffffff; padding: 30px; border: 1px solid #e5e7eb; border-top: none;">
                        ...
                        <p style="font-size: 16px; color: #374151; line-height: 1.6;">
                            Great news! We have received your payment and your visa application is now being processed.
                        </p>
                        
                        <div style="background: #dbeafe; border-left: 4px solid #2563eb; padding: 20px; margin: 25px 0; border-radius: 4px;">
                        ...
                        <div style="background: #f9fafb; padding: 20px; border-radius: 8px; margin: 25px 0;">
                            ...
                            <p style="margin: 8px 0; color: #4b5563;"><strong>Status:</strong> Processing</p>
                        </div>
                        
                        <p style="font-size: 16px; color: #374151; line-height: 1.6;">
                            Our team is working diligently to process your visa application. We will notify you once your visa is ready.
                        </p>
        ...
        return Response.json({ 
            success: true, 
            message: 'Processing notification sent',
            emailId: emailResult.id 
        });
    } catch (error) {
        console.error(`Error in notifyCustomerOrderProcessing: ${error.message}`);
        return Response.json({ 
            error: error.message
        }, { status: 500 });
    }
